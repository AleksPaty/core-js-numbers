...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const xLine = Math.abs(x2 - x1);
  const yLine = Math.abs(y2 - y1);
  if (xLine > 1 || yLine > 1) {
    return Math.PI;
  }
  if (xLine === 1 || yLine === 1) {
...
function roundToPowerOfTen(num, pow) {
  const exponential = 10 ** pow;
  return Math.round(num / exponential) * exponential;
}
...
function getFibonacciNumber(index) {
  ...
  while (fibonacciNumbers.length - 1 < index) {
    const num = fibonacciNumbers.slice(-2).reduce((prev, cur) => prev + cur);
